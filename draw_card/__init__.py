...
from .prc_handle import update_prc_info, prc_draw
...
prc = on_regex('.*?(prc|公主连结|公主连接|公主链接)[1-9|一][0-9]{0,2}[抽|井]', rule=is_switch('prc'), priority=5, block=True)
prc_update = on_keyword({'更新prc信息', '更新公主连结信息'}, permission=SUPERUSER, priority=1, block=True)
...
async def _(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if msg.find('1井') != -1 or msg.find('一井') != -1:
        ...
        rmsg = re.search(r'.*?(prc|公主连结)(.*)[抽|井]', msg)
        if rmsg:
            num, flag = check_num(rmsg.group(2), 300)
            if not flag:
                await prc.finish(num, at_sender=True)
        else:
            return
    await prc.send(await prc_draw(int(num)), at_sender=True)
