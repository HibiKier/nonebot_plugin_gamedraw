                        ...
                        for char in contents[1:]:
                            try:
                                data = await retrieve_char_data(
                                    char, game_name, data, session, index
                                )
                            except AttributeError:
                                continue
                        index += 1
...
async def _last_check(
    data: dict, game_name: str, session: aiohttp.ClientSession
) -> dict:
    if game_name == "azur":
        idx = 1
        for url in [
        ...
        for key in data.keys():
            tasks.append(
                asyncio.ensure_future(
                    _async_update_azur_extra_info(key, session, semaphore)
                )
            )
...
async def _async_update_azur_extra_info(
    key: str, session: aiohttp.ClientSession, semaphore
):
    if key[-1] == "改":
        ...
        for i in range(20):
            try:
                async with session.get(
                    f"https://wiki.biligame.com/blhx/{key}", timeout=7
                ) as res:
